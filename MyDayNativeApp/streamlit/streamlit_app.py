...
def main_page():
    ...
    chatPrompt = f'''
        ...
        This person on average ate {cals_consumed} calories.
        This person on average burned {cals_burned} calories.
    ...
    # st.header("Diet & Activity Trends")
    # st.markdown(f'''A healthy diet is of paramount importance for overall well-being to provide essential nutrients, vitamins, and minerals that support 
    #                 bodily functions, boost the immune system, and promote optimal growth 
    #                 and development. Below are your Macros and diet information.''')


    # WEIGHT CHART
    weight = agg[['DATE','BODYMASS']]
    weight_chart =alt.Chart(weight).mark_line().encode(
        x=alt.X('DATE', title=None,  scale=alt.Scale(padding=0)),
        y=alt.Y('BODYMASS', axis=None, scale=alt.Scale(domain=[weight['BODYMASS'].min(), weight['BODYMASS'].max()], padding=0) )
    )

    weight_text = alt.Chart(weight).mark_text( color='#094044', baseline='top', align='right', fontSize=15,fontWeight=900).encode(
        x = alt.X('DATE:T'),
        y = alt.Y('BODYMASS'),
        text = 'BODYMASS'
    ) 
    ...
    weight_plot = alt.layer(weight_chart, weight_text).properties(
    ...
    'date': agg['DATE'] + timedelta(days=1),
    ...
    heart_rate_plot = session.sql(f""" select DISTINCT DATE_TRUNC('DAY', STARTTIME)::DATE +1 AS DATE, VALUE::INT AS HR
                        from hk.heartrate
                        WHERE ID ='{pin}' 
                        and STARTTIME between '{start_date}' and '{end_date}' 
                        order by DATE asc""").to_pandas()
    ...
    chart_for_sleep = alt.Chart(sleep_dataframe).mark_tick().encode(
        ...
        x= alt.Y('monthdate(Y):O', title=None),
